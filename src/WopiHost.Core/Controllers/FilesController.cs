    ...
    ICobaltProcessor? cobaltProcessor = null) : ControllerBase
{
    ...
    private IActionResult HandleLockOrPut(string id, string? oldLockIdentifier, string? newLockIdentifier, bool lockAcquired, WopiLockInfo? existingLock)
    {
        ...
        else
        {
            if (lockAcquired)
            {
                ...
                if (AreLocksCompatible(existingLock.LockId, oldLockIdentifier))
                {
                    if (string.IsNullOrWhiteSpace(newLockIdentifier))
                    {
                        return new LockMismatchResult(Response, reason: "Missing new lock identifier");
    ...
    private IActionResult HandleUnlock(string id, string? newLockIdentifier, bool lockAcquired, WopiLockInfo? existingLock)
    {
        ...
        if (lockAcquired)
        {
            ...
            if (AreLocksCompatible(existingLock.LockId, newLockIdentifier))
            {
    ...
    {
        ArgumentNullException.ThrowIfNull(lockProvider);
        if (AreLocksCompatible(existingLock.LockId, newLock))
        {
            // File is currently locked and the lock ids are compatible, refresh lock (extend the lock timeout)
            if (lockProvider.RefreshLock(existingLock.FileId, newLock))
            {
                return Ok();
            }
            else
    ...
    /// </summary>
    private static bool AreLocksCompatible(string? lockId1, string? lockId2)
    {
    ...
    /// </summary>
    private static string ExtractSessionId(string lockId)
    {
        ...
        try
        {
            ...
            using var document = System.Text.Json.JsonDocument.Parse(lockId);
