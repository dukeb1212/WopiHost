...
public class WopiProofValidator : IWopiProofValidator
{
    ...
    public async Task<bool> ValidateProofAsync(HttpContext httpContext, string accessToken)
    {
        ...
        {
            _logger.LogWarning("WOPI proof validation skipped in development environment");
            return true;
        }
        try
        {
    ...
    private bool VerifyProof(byte[] expectedProof, string proofFromRequest, string proofFromDiscovery)
    {
        ...
        try
        {
            _logger.LogInformation(
                "Proof validation data - Expected proof length: {ExpectedProofLength}, " +
                "Proof from request: {ProofRequest}, " +
                "Proof from discovery: {ProofDiscovery}",
                expectedProof.Length,
                proofFromRequest.Substring(0, Math.Min(10, proofFromRequest.Length)),
                proofFromDiscovery.Substring(0, Math.Min(10, proofFromDiscovery.Length)));
